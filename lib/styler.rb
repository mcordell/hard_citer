...
class Styler
  ...
  def get_bibliography_lines(bibliography_array,csl_style)
    ...
    bibliography_array.each do |cite_key,cite_match|
      ...
      if entry.nil?
        ...
        cite_text=CiteProc.process cite_match.citation.to_citeproc, style: csl_style, format: :html
        strip_extra_papers_brackets cite_text
      end

      out_lines.push "<li><a name=\"bibliography_#{cite_match.bib_number}\">"+cite_text+"</a></li>"
    end
    out_lines.push "</ol>"
  end
  ...
  def style_line(line,citations)
    ...
    citations.each do |cite_match|
      if cite_match[0].is_a?(CiteMatch)
        output,offset=single_cite(cite_match[0], cite_match[1]-pos_off_set, line)
        pos_off_set+=offset
  ...
  def multi_cite(cite_match_array,line,pos_offset)
    ...
    cite_match_array.each_with_index do |cite_match,index|
      ...
      output_line+=output
      unless index == cite_match.size-1
        output_line+=@multi_separator
      end
      pos_offset+=off_set
    end
  ...
  def single_cite(citation,pos,line)
    ...
    in_text_citation="<sup><a href=\"#bibliography_#{citation.bib_number}\">#{citation.bib_number}</a></sup>"
